...
interface TripsDataProps {
  ...
  onCreateTrip: (template?: DestinationTemplate) => void;
}
...
export function TripsCollection({
  ...
  return (
    <Surface>
      ...
      {/* Gamified Traveller Level & Progress Card */}
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-1">
          <TravellerLevelCard
            tripsCount={trips.length || 3}
            onExplore={() => onCreateTrip()}
          />
        </div>
        <div className="lg:col-span-2">
          <PopularDestinationsCarousel
            onSelectDestination={(dest) => onCreateTrip(dest)}
          />
        </div>
      </div>
        ...
        <div className="mt-4">
          <div className="mb-3 flex items-center justify-between">
            <h3 className="font-heading text-lg font-bold text-foreground">
              Your Itineraries & Group Escapes
            </h3>
          </div>
          <Tabs
