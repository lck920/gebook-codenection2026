...
const appPath = resolve(root, "backend/src/interfaces/http/app.ts");
...
const dtoPath = resolve(root, "backend/src/application/dto.ts");
const opsSchemasPath = resolve(
  root,
  "backend/src/application/trip/ops/schemas.ts",
);
...
describe("client API documentation coverage", () => {
  ...
  it("documents vote/comment as edit (loadEditable) and agent addressed semantics", () => {
    const useCases = readFileSync(
      resolve(root, "backend/src/application/use-cases.ts"),
      "utf8",
    );
    ...
    const agentService = readFileSync(
      resolve(root, "backend/src/application/agent/agent-service.ts"),
      "utf8",
    );
